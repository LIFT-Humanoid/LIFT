...
class G1GoFast(RlwamEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.action_scale = jp.array([0.54754645, 0.35066146, 0.54754645, 0.35066146, 0.43857732, 0.43857732, 
        ...
        self.reward_config=config_dict.create(
            scales=config_dict.create(
                ...
                # feet_slip=-0.1,
                # feet_vel_z=-1.0,     # disabled in Isaac config
                # feet_yaw_diff=-1.0,
                # feet_yaw_mean=-1.0,
                # feet_roll=-0.1 * 10.0,  # original -0.1
                # feet_distance=-1.0 * 10.0,  # original -1.0
        ...
        self.default_joint_pd_target = g1Utils.ALL_STANDING_JOINT_ANGLES
        self.commands = jp.array([vel_x_command, 0.0, 0.0, 0.0])
        self.terminate_height_min = 0.3
        self.terminate_height_max = 1.0
        self.terminate_lin_vel_max = 10.0
        self.terminate_ang_vel_max = 10.0


        self._soft_joint_pos_limit_factor = 0.95
        c = (g1Utils.LOWER_JOINT_LIMITS + g1Utils.UPPER_JOINT_LIMITS) / 2
        r = g1Utils.UPPER_JOINT_LIMITS - g1Utils.LOWER_JOINT_LIMITS
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        def f(info_list, _):
            ...
            torque, cmd = self.torque_pd_control(action, g1Utils.KP, g1Utils.KD, q, qd)
    ...
    def _is_healthy(self, next_norm_obs: jp.ndarray) -> jp.ndarray:
        ...
        condition = jp.logical_and(condition, jp.all(next_obs[self._priv_q_idxs] < g1Utils.UPPER_JOINT_LIMITS*1.5))
        condition = jp.logical_and(condition, jp.all(next_obs[self._priv_q_idxs] > g1Utils.LOWER_JOINT_LIMITS*1.5))
        condition = jp.logical_and(condition, jp.all(next_obs[self._priv_qd_idxs] < g1Utils.MOTOR_VEL_LIMIT*4))
        condition = jp.logical_and(condition, jp.all(next_obs[self._priv_qd_idxs] > -g1Utils.MOTOR_VEL_LIMIT*4))
    ...
    def _get_gait_phase(self, sin_curve):
        ...
        stance_mask = jp.array([sin_curve >= 0, sin_curve < 0])

        # Double support phase
        stance_mask = jp.where(jp.abs(sin_curve) < 0.1, True, stance_mask)

        return stance_mask
    
    def compute_ref_state(self, obs: jp.ndarray,):
        sin_pos = obs[self._sin_phase_idx]
        sin_pos_l = sin_pos.copy()
        sin_pos_r = sin_pos.copy()

        pdes = jp.zeros((self.action_size,))
        
        # left stand
        sin_pos_l = jp.where(sin_pos_l > 0, 0.0, sin_pos_l)
        sin_pos_r = jp.where(sin_pos_r < 0, 0.0, sin_pos_r)
        ...
        pdes = jp.where(jp.abs(sin_pos) < 0.1, 0.0, pdes)

        pdes += g1Utils.ALL_STANDING_JOINT_ANGLES
    ...
    def low_level_control_hardware(self, action: jp.ndarray,
        ...
        # action: (10,), obs:(32, )
        # ref_action, stance_mask = self.compute_ref_state(obs[self._sin_phase_idx])

        q_des = action * self.action_scale   + g1Utils.ALL_STANDING_JOINT_ANGLES
        #q_des = action*0.25   + g1Utils.ALL_STANDING_JOINT_ANGLES
    ...
    def _limit_Kp(self, obs, q_des, qd_des, Kp, Kd):
        ...
        Kp = jp.where(
            torque_theo > g1Utils.MOTOR_TORQUE_LIMIT,
            (g1Utils.MOTOR_TORQUE_LIMIT - Kd*(qd_des - qd))/q_err, Kp)
        Kp = jp.where(
            torque_theo < -g1Utils.MOTOR_TORQUE_LIMIT,
            (-g1Utils.MOTOR_TORQUE_LIMIT - Kd*(qd_des - qd))/q_err, Kp)
        return Kp
