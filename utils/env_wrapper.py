...
from utils.types import State, ObservationSize
...
class FrameStackWrapper(Wrapper):
    ...
    def __init__(self, env: Env, obs_history_length: int, priv_obs_history_length: int, noise_std: jp.ndarray = 0.0):
        super().__init__(env)
...
class DomainRandomizationVmapWrapper(Wrapper):
    ...
    def __init__(
        self,
        env: Env,
        randomization_fn: Callable[[Any], Tuple[Any, Any]],
    ):
        super().__init__(env)
        self._sys_v, self._in_axes = randomization_fn(self.sys)

    def _env_fn(self, sys: Any) -> Env:
        env = self.env
